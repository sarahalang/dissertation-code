...
def remove_namespace(string):
    """
    Removes namespace from URI for easier inspection of results.
    """
...
def getConceptAsURI(string):
    """
    Prefaces concept with names spaces and returns it as URIRef.
    """
...
def uriIsInGraph(graph, uri):
    """
    Checks if a given URI is in the graph.
    """
...
def getContexts(graph, uri):
    """
    Function to calculate all contexts that one concept has in the KOS.
    """
    uri_isA = []
    contexts = []
...
def normalizeSpace(xml):
    """
    Will normalize space in XML, meaning it places spaces around dots, dashes
    <> and normalizes multiple whitespace or linebreaks into one whitespace.
    """
    xml = re.sub(r'[>]', '> ', xml)
    xml = re.sub(r'[<]', ' <', xml)
    xml = re.sub(r'[.]', ' . ', xml)
...
def get_dictionary_key(dictionary, value):
    """
    Function to find a dictionary key by its value.
    """
    matching_keys = []
...
def cleanupXMLUnicode(xml):
    """
    Before saving to XML, certain illegal characters need to be removed with this function.
    Closely adapted from this Stackoverflow answer:
